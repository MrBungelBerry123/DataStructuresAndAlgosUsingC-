...
* This data structure keeps track of the max k elements.
*/

#ifndef _TOP_K_HEAP_H_
#define _TOP_K_HEAP_H_
#include "MaxHeap.h"
...
namespace ds {
	...
	public:
		TopKHeap() {
			...
			this->k = LEAST_K;
			this->heap = new ds::MaxHeap<T>();
			this->currentSize = 0;
		}

		TopKHeap(const int k) {
			this->arr = new T[k];
			this->k = k;
			this->heap = new ds::MaxHeap<T>();
			this->currentSize = 0;
		...
		TopKHeap(const int k, const int size) {
			this->arr = new T[k];
			this->k = k;
			this->heap = new ds::MaxHeap<T>(size);
			this->currentSize = 0;
		}
